...
### Fixed
- Left-aligned the `info()` table headers with their column values
- Added regression coverage for rendered header and value alignment
...
### Added
- A four-column `info()` reference view: level, module, function, and detail
- Short positional level filters such as `info("core")` and `info("finance")`
- Explicit sorting through `info("level")`, `info("module")`, `info("function")`, and `info("detail")`
- Regression tests for level ordering, filtering, and default behavior

### Changed
- `info()` now defaults to the same package-level grouping as `info("level")`
- Input/output functions are grouped under the `input_output` level
- Function documentation summaries are displayed directly in the `detail` column

### Fixed
- Declared Jinja2 as a runtime dependency for pandas styled table output
